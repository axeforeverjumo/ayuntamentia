...
def create(
    ...
    with get_db() as conn:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute(
            """INSERT INTO subscripciones (user_id, nombre, temas, municipios, canal, cron_expr, activo)
               VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING *""",
            (user.user_id, body.nombre, body.temas, body.municipios, body.canal, body.cron_expr, body.activo),
        )
        row = cur.fetchone()
    log_usage(user, "subscripcion_create", payload={"id": row["id"], "temas": body.temas}, request=request)
    return row
    ...
    with get_db() as conn:
        ...
        cur.execute(
            """UPDATE subscripciones SET
                nombre=%s, temas=%s, municipios=%s, canal=%s, cron_expr=%s, activo=%s
               WHERE id=%s AND user_id=%s RETURNING *""",
            (body.nombre, body.temas, body.municipios, body.canal, body.cron_expr, body.activo,
             sub_id, user.user_id),
        )
        row = cur.fetchone()
        if not row:
